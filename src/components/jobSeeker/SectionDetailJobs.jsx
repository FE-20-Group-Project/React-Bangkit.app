...
function SectionDetailJobs({jobs, detailJobs, setIsLoading}) {
    const {session} = useSelector( state => state.userSession );
    const navigate = useNavigate();

    const redirectLoker = (id) => {
        navigate(`/jobs/${id}`);
        setIsLoading(true);
    }
